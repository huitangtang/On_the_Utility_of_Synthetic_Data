...
def opts():
    parser = argparse.ArgumentParser(description='MCD', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    # Dataset options
    ...
    #parser.add_argument('--lr_plan', type=str, default='dann', choices=['dann', 'constant'], help='LR decay plan.')
    ...
    #parser.add_argument('--schedule', type=int, nargs='+', default=[79, 119], help='Decrease learning rate at these epochs.')
    parser.add_argument('--gamma', type=float, default=0.1, help='LR is multiplied by gamma on schedule.')
    parser.add_argument('--concat_st', action='store_true', help='Whether concatenating Bs and Bt as one batch.')
    parser.add_argument('--num_k', type=int, default=4, help='Number of steps to train generator.')
    parser.add_argument('--setting', type=str, default='original', help='Flag for different settings.')
    ...
    parser.add_argument('--feat_proj', action='store_true', help='Whether adding the feature projection layer.')
    parser.add_argument('--num_fc', type=int, default=3, help='Three fc layers are used in MCD as default.')
    ...
    args.log += args.source + '2' + args.target + '_bs' + str(args.batch_size_s) + '_lr' + str(args.lr) + '_' + args.setting + '_' + args.arch + '_pretrain' + str(args.pretrained)
